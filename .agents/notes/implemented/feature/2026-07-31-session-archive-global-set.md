...
The session row menu in the sidebar workspace browser carried a purely visual "Delete session" placeholder (no handler). The immediate product need was **archive**, a non-destructive hide: the session log and its workspace accounting stay untouched; the session merely disappears from every grouping surface (workspace groups, Ungrouped, search, the flat list). The archive record needs a home: an Ungrouped session belongs to no workspace entity, so a per-workspace field cannot carry it. [Permanent Web Session deletion](2026-09-04-web-session-permanent-deletion.md) later adds a separate destructive action without changing Archive semantics.
...
- UI: the original visual-only `delete` row becomes `archive` (label "Archive session", non-danger styling, no confirmation dialog — a non-destructive action whose worst misfire is list hiding); [permanent deletion](2026-09-04-web-session-permanent-deletion.md) later adds its own confirmed menu row. Filtering is one extra arm in `tree.ts`'s `sessionVisible` predicate, with `deriveGroups`/`deriveFlat` taking an `archived` set parameter so all four surfaces (group loop, stray bucket, search, flat) share one source.
