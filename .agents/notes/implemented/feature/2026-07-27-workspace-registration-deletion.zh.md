...
菜单、`Modal` 和按钮保留现有结构与设计 token。[Web 会话永久删除](2026-09-04-web-session-permanent-deletion.zh.md)是另一项独立的行内操作，不改变此处 Workspace 的保留边界。

## 考虑过的替代方案

**级联删除会话。** 不予采纳，因为 Workspace 注册记录不拥有会话持久化，且产品需求是将历史记录保留在 Ungrouped 下。逐 Session 删除在 [Web 会话永久删除](2026-09-04-web-session-permanent-deletion.zh.md)中拥有独立生命周期、所有权检查与明确 UI。
