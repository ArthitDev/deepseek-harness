...
- **挂载一旦成功即持续供职，直到组装文件的 stamp 变化。** 运行中会话加入的组装必须在其文件被修改或删除后继续存活；每个代际记录文件 stamp（mtime + 大小），会话发现当前代际已陈旧时，会开启下一个代际。浏览器写入会主动丢弃 standing 指针；其他文件编辑器所作的修改仍由 stamp 检出。已加入的会话保持其代际，被替代的代际只由整树卸载回收——刻意为之，上限取决于编辑频率，已记入包的 Known Limitations。
