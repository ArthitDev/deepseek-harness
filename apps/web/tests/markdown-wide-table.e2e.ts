...
describe('web e2e: markdown tables fill the column, wide ones break out and scroll', () => {
  ...
  it('reports the same relations on a high-DPI page', async () => {
    ...
    try {
      ...
      const groupRow = hidpiPage.locator('[data-workspace-group] > [role="treeitem"]').first()
      await groupRow.waitFor({ timeout: 15_000 })
      if (await groupRow.getAttribute('aria-expanded') !== 'true') await groupRow.click()
      const sessionRow = hidpiPage.locator('[data-workspace-group] [role="treeitem"][aria-selected]').first()
      await sessionRow.waitFor({ timeout: 10_000 })
      await sessionRow.click()
