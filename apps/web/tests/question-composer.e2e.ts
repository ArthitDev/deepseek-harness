...
describe.skipIf(MODE === 'record')('web e2e: cancelled question transcript', () => {
  ...
  beforeAll(async () => {
    ...
    const groupRow = cancelledPage.locator('[data-workspace-group] > [role="treeitem"]').first()
    await groupRow.waitFor({ timeout: 15_000 })
    if (await groupRow.getAttribute('aria-expanded') !== 'true') await groupRow.click()
    const sessionRow = cancelledPage.locator('[data-workspace-group] [role="treeitem"][aria-selected]').first()
