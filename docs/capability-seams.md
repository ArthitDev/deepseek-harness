...
| `ctx.reconEngineOptions` | `core` | [`recon-engine`](../packages/pentest/recon-engine) | - | [`recon-engine`](../packages/pentest/recon-engine) | - | Carries the resolved evidence directory and execution scope from the plugin into its Remote controller. |
...
| `ctx.remoteMachines` | `core` | [`remote-machines`](../packages/remote/remote-machines) | - | - | - | Owns saved profiles, lifetime-only passwords, host-key trust, and the connection pool used by its filesystem and subprocess providers. |
