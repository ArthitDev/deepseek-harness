...
`session.delete` stops an active Agent only when this Web controller owns its exact handle, then removes the stored Session log and Workspace references; another owner or a subagent identity is rejected, and project files are never removed.
