...
`dsh-tool-web` 让模型通过 `ctx.web` 调用 `web_search` 与 `web_fetch`。部署可以独立启用任一工具。成功结果把提供方控制的文本标记为外部不可信数据，HTML 转换会删除活动或隐藏内容。提供方缺失时，工具不会隐藏，而是返回模型可读的结构化错误。部署的超时策略强制执行每个工具的预算。
