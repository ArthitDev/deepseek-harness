...
describe('registry-global session archive', () => {
  ...
  it('forgets a deleted Session from accounting and archive state without touching its directory', async () => {
...
describe('first-use Workspace preparation', () => {
  ...
  it('rejects a relative candidate before creating its directory', async () => {
    const h = await firstUse()
    const candidate = join('relative', basename(h.directoryRoot))
    ...
    await expect(realpath(resolve(candidate))).rejects.toMatchObject({ code: 'ENOENT' })
