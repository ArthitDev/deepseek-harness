...
Web GUI 通过 `/model` 与 composer 提供模型选择。两处都按提供方分组显示当前会话可用的模型，并显示提供方名称和适配器定义的推理强度。完整的提供方、模型与推理强度选择在下一个请求边界生效；活动步骤保留已组装的路由。没有适配器服务该路由时，composer 保持停用，直到路由恢复可用。
