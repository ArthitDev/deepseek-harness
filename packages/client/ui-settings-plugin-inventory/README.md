...
The **Plugin list** tab shows agent-preset and global plugins in collapsible groups. It loads on first selection, reports enablement and failures, and lets search open matching groups or presets. Preset-provided global entries identify which presets enable them. Loading, empty, no-match, and retry states remain inside the tab; without a preset roster, only the expanded global group appears.
