...
Shield Break Agent also accepts legacy `text` when `prefix` is absent, normalizing it in memory without rewriting the preset. An invalid explicit `prefix` still fails validation. The preset editor saves `prefix` and preserves `suffix` and other fields.
