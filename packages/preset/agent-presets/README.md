...
`dsh-agent-presets` gives each session tools, prompt sections, and skills from one `agent.cordis.yml` preset. It lists shipped and user presets, reports broken entries, and supports direct creation, duplication, deletion, defaults, and per-model bindings. A blank session may switch preset; a started session keeps its composition. Presets inherit every privilege of their plugins, so author only presets you trust.
...
The authenticated browser can create a preset by entering an id, an optional display name, and a system prompt. Direct creation copies the current default preset's tools, skill directories, assets, and metadata into the first `user` root, replaces only its persona prompt, and succeeds only after the complete write finishes. Duplication remains available when the new preset should start as an exact copy of a selected source. Later prompt edits send only the preset id and text; the Host resolves the exact path and refuses shipped presets or paths outside the first user root. A composition may contain executable `!!js`, so creating or editing one carries the same trust as shell access.
...
- [Per-session agent presets note](../../../.agents/notes/archived/architecture/2026-08-03-per-session-agent-presets.md) — design rationale and alternatives.
