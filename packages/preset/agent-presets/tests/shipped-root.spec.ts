...
describe('the shipped preset root', () => {
  ...
  it('prepends the shipped root before configured roots and the derived user root', async () => {
    ...
    expect(agentPresets(ctx).roots.map(root => root.path)).toEqual([
      SHIPPED_PRESET_ROOT,
      SYSTEM_ROOT,
      expect.stringContaining('.agent-presets'),
    ])
    expect(agentPresets(ctx).roots[0]).toEqual({ path: SHIPPED_PRESET_ROOT, trust: 'system' })
    ...
    // shipped one.
    const minimal = (await agentPresets(ctx).list()).find(preset => preset.id === 'minimal')
  ...
  it('mounts a roster without the shipped set when includeShippedRoot is false', async () => {
    ...
    expect(agentPresets(ctx).roots).toEqual([{ path: SYSTEM_ROOT, trust: 'system' }])
    const minimal = (await agentPresets(ctx).list()).find(preset => preset.id === 'minimal')
