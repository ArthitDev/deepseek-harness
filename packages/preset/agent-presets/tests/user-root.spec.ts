...
describe('the harness-home preset root', () => {
  ...
  it('is discovered without any app configuring it', async () => {
    ...
    expect((await agentPresets(ctx).resolve('mine')).path)
  ...
  it('makes a roster with only a system root authorable, and receives the copy', async () => {
    ...
    expect(agentPresets(ctx).authorable).toBe(true)
    await agentPresets(ctx).copy('standard', 'copied')
  ...
  it('sorts after every configured root, so a shipped id still shadows a home directory', async () => {
    ...
    expect((await agentPresets(ctx).resolve('standard')).trust).toBe('system')
    await expect(agentPresets(ctx).copy('standard', 'standard')).rejects.toThrow(/already exists/)
  })
  ...
  it('is absent under includeUserRoot: false, which leaves the roster unauthorable', async () => {
    ...
    expect((await agentPresets(ctx).list()).map(preset => preset.id)).not.toContain('mine')
    expect(agentPresets(ctx).authorable).toBe(false)
    await expect(agentPresets(ctx).copy('standard', 'mine'))
