...
describe('rows naming a plugin that cannot be resolved', () => {
  ...
  it('uses the host lookup when the profile routes packages in memory', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsh-presets-routed-'))
    roots.push(root)
    await mkdir(join(root, 'probe'))
    await writeFile(join(root, 'probe', COMPOSITION_FILE), "- id: p\n  name: '@scope/pkg'\n")
    const lookup = vi.fn((specifier: string) => specifier === '@scope/pkg')

    const [preset] = await scanRoot({ path: root, trust: 'user' }, HARNESS, lookup)

    expect(preset?.broken).toBeUndefined()
    expect(lookup).toHaveBeenCalledWith('@scope/pkg', HARNESS)
  })
