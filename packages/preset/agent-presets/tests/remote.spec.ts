...
async function agentOn(ctx: Context, id: string, presetId?: string): Promise<Agent> {
  const handle = await ctx.agents.create({
    ...
    setup: async (agentCtx: Context) => void await agentPresets(ctx).mount(agentCtx, presetId),
  })
  return handle.agent
}
...
describe('reading one composition', () => {
  it('rejects an empty id before resolving it', async () => {
    const ctx = await harness()
    const resolve = vi.spyOn(agentPresets(ctx), 'resolve')

    await expect(agentPresets(ctx).readDocument(''))
  ...
  it('answers the stored text with the row it belongs to', async () => {
    ...
    const document = await agentPresets(ctx).readDocument('standard')
    ...
    expect(document).toEqual({
      ...
      trust: 'system',
      content: await agentPresets(ctx).read('standard'),
    })
  })
  ...
  it('carries the display metadata a preset published', async () => {
    ...
    const document = await agentPresets(ctx).readDocument('documented')
  ...
  it('refuses an id no root supplies', async () => {
    ...
    const failure = await remoteFailure(agentPresets(ctx).readDocument('never-existed'))
  ...
  it('raises an unrelated read failure exactly as it was thrown', async () => {
    ...
    vi.spyOn(agentPresets(ctx), 'read').mockRejectedValueOnce(thrown)

    await expect(agentPresets(ctx).readDocument('standard')).rejects.toBe(thrown)
  })
})
...
describe('authoring over Remote', () => {
  it('rejects empty source, target, and delete ids before authoring', async () => {
    const ctx = await harness()
    const copy = vi.spyOn(agentPresets(ctx), 'copy')
    const write = vi.spyOn(agentPresets(ctx), 'write')
    const remove = vi.spyOn(agentPresets(ctx), 'remove')

    for (const operation of [
      () => agentPresets(ctx).remoteExportCopy('', 'mine'),
      () => agentPresets(ctx).remoteExportCopy('standard', ''),
      () => agentPresets(ctx).remoteExportWrite('', VALID),
      () => agentPresets(ctx).remoteExportDelete(''),
    ]) {
  ...
  it('copies and deletes through the Remote adapters', async () => {
    ...
    await agentPresets(ctx).remoteExportCopy('standard', 'mine', '我的模式')
    expect((await agentPresets(ctx).resolve('mine')).name).toBe('我的模式')

    await agentPresets(ctx).remoteExportDelete('mine')
    await expect(agentPresets(ctx).resolve('mine')).rejects.toThrow(/not found/)
  })
  ...
  it('creates a preset with supplied composition through the Remote adapter', async () => {
    ...
    await agentPresets(ctx).remoteExportCreate('standard', 'direct', 'Direct', content)

    expect(await readFile(join(userRoot, 'direct', COMPOSITION_FILE), 'utf8')).toBe(content)
    expect((await agentPresets(ctx).resolve('direct')).name).toBe('Direct')
  })
  ...
  it('writes a custom composition and refuses a shipped preset', async () => {
    ...
    await agentPresets(ctx).remoteExportCopy('standard', 'mine')
    const changed = `${VALID}# changed\n`

    await agentPresets(ctx).remoteExportWrite('mine', changed)

    expect(await readFile(join(userRoot, 'mine', COMPOSITION_FILE), 'utf8')).toBe(changed)
    const failure = await remoteFailure(agentPresets(ctx).remoteExportWrite('standard', changed))
    expect(failure).toMatchObject({
  ...
  it('preserves not-found details for an unknown copy source', async () => {
    ...
    const failure = await remoteFailure(agentPresets(ctx).remoteExportCopy('never-existed', 'mine'))
  ...
  it('preserves invalid-id and occupied-id failures', async () => {
    ...
    const invalid = await remoteFailure(agentPresets(ctx).remoteExportCopy('standard', '../escape'))
    ...
    const occupied = await remoteFailure(agentPresets(ctx).remoteExportCopy('standard', 'minimal'))
  ...
  it('keeps the requested id when no writable root exists', async () => {
    ...
    const failure = await remoteFailure(agentPresets(ctx).remoteExportCopy('standard', 'mine'))
  ...
  it('preserves read-only and not-found delete failures', async () => {
    ...
    const readOnly = await remoteFailure(agentPresets(ctx).remoteExportDelete('standard'))
    ...
    const missing = await remoteFailure(agentPresets(ctx).remoteExportDelete('never-existed'))
  ...
  it('raises an unrelated authoring failure exactly as it was thrown', async () => {
    ...
    vi.spyOn(agentPresets(ctx), 'copy').mockRejectedValueOnce(thrown)

    await expect(agentPresets(ctx).remoteExportCopy('standard', 'mine')).rejects.toBe(thrown)
  })
})
...
describe('switching one session\'s composition', () => {
  it('rejects an empty preset id before queuing a switch', async () => {
    ...
    const recompose = vi.spyOn(agentPresets(ctx), 'recompose')

    await expect(agentPresets(ctx).select(agent, ''))
      .rejects.toMatchObject({ code: 'gateway/bad-request' })
  ...
  it('recomposes a blank session and records what it now runs', async () => {
    ...
    expect(await agentPresets(ctx).select(agent, 'minimal')).toBe('minimal')
  ...
  it('serializes two concurrent switches on one session', async () => {
    ...
    await Promise.all([
      agentPresets(ctx).select(agent, 'minimal'),
      agentPresets(ctx).select(agent, 'standard'),
    ])
  ...
  it('leaves the session on its composition when the named preset is unknown', async () => {
    ...
    const failure = await remoteFailure(agentPresets(ctx).select(agent, 'nope'))
  ...
  it('serves a later switch after one was refused', async () => {
    ...
    await expect(agentPresets(ctx).select(agent, 'nope')).rejects.toThrow(/not found/)
  ...
  it('reports an unusable composition with its discovery reason', async () => {
    ...
    const failure = await remoteFailure(agentPresets(ctx).select(agent, 'damaged'))
  ...
  it('raises an unrelated switch failure exactly as it was thrown', async () => {
    ...
    vi.spyOn(agentPresets(ctx), 'recompose').mockRejectedValueOnce(thrown)

    await expect(agentPresets(ctx).select(agent, 'minimal')).rejects.toBe(thrown)
  })
})
