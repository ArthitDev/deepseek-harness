...
/** Host package lookup used when a profile routes dependencies in memory. */
type PackageLookup = (specifier: string, parentURL: string) => boolean
 ...
 *
 * The filesystem walk covers physical installs; the host lookup covers
 * profiles whose dependency graph is routed in memory.
 * @param name - the package specifier, possibly carrying a subpath.
 * @param base - the URL to walk up from.
 * @param lookup - optional Host resolver for an in-memory profile graph.
 * @returns true when the package directory is installed above `base`.
 */
function packageInstalled(name: string, base: string, lookup?: PackageLookup): boolean {
  ...
  return lookup?.(name, base) ?? false
}

/**
 * Whether one classified row names a module that exists, importing nothing.
 *
 * Each kind is checked by what actually answers it. A package name uses
 * Node's resolver without evaluation, while relative and `file:` specifiers
 * are statted because both name one file.
 * Nothing is evaluated either way, so a row is judged without its plugin
 * observing that discovery looked.
 *
 * The optional host lookup participates in profile runtime interception. That
 * matters when the dependency graph is routed in memory and deliberately
 * leaves the profile directory unwritten.
...
async function rowResolves(
  ...
  if (row.kind === 'package') return isBuiltin(row.specifier) || packageInstalled(row.specifier, harnessBase, lookup)
...
async function unresolvableRows(
  ...
  for (const [index, entry] of rows.entries()) {
    ...
    if (row.group === true) {
      found.push(...await unresolvableRows(row.config as readonly unknown[], presetBase, harnessBase, lookup, positional))
      continue
    }
    if (await rowResolves(classifyRowSpecifier(row.name), presetBase, harnessBase, lookup)) continue
...
async function compositionProblem(
  ...
  const unresolvable = await unresolvableRows(rows as readonly unknown[], presetBase, harnessBase, lookup)
...
export async function scanRoot(
  ...
  for (const child of children) {
    ...
    const broken = await isFile(path)
      ? await compositionProblem(path, harnessBase, lookup)
...
export async function discoverPresets(
  ...
  for (const root of roots) {
    for (const preset of await scanRoot(root, harnessBase, lookup)) {
