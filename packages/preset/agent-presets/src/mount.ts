...
export async function inactiveRows(tree: EntryTree): Promise<string[]> {
  ...
  for (const entry of tree.entries()) {
    ...
    try {
      ...
      const groups = []
      let parent = entry.parent.ctx.fiber.entry
      while (parent) {
        if (parent.options.group && !failedGroups.has(parent.id)) groups.unshift(parent)
        parent = parent.parent.ctx.fiber.entry
      }
      for (const group of groups) {
        failedGroups.add(group.id)
        lines.push(`${group.options.id} (${group.options.name}): child row did not activate`)
      }
