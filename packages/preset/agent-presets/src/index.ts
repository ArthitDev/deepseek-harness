...
import {
  ...
  type AgentPresetRosterComposition,
...
export type {
  AgentPresetRosterComposition, AgentPresetRosterCompositionRow, CompositionRowEnablement,
} from './composition-inventory.ts'
...
export class AgentPresets extends TypertRemoteService {
  ...
  /**
   * Optional settings service, held for clearing bindings to a deleted preset.
   */
  ...
  private selectionPolicy(): { enabled: boolean; defaultId: string } {
    const enabled = this.config.modeSelectionEnabled?.get() ?? true
    return {
      enabled,
  ...
  async list(): Promise<AgentPreset[]> {
    const packages = this.selfCtx.get('pluginPackages')
    return await discoverPresets(
      this.resolvedRoots,
      this.harnessBase,
      packages === undefined ? undefined : (specifier, parentURL) => packages.packageOf(specifier, parentURL) !== undefined,
    )
  }
  ...
  async remoteExportList(): Promise<AgentPresetRoster> {
    ...
    return {
      ...
      models,
      modelPresets: this.config.models?.get() ?? {},
   ...
   */
  async compositionInventory(): Promise<AgentPresetRosterComposition[]> {
    ...
    const found: AgentPresetRosterComposition[] = []
  ...
  async remove(id: string): Promise<void> {
    ...
    const selectedDefault = this.config.selectedDefault?.get()
    const models = this.config.models?.get()
    const ops = [
      ...selectedDefault === id ? [{ op: 'unset' as const, path: ['selectedDefault'] }] : [],
      ...Object.entries(models ?? {}).flatMap(([provider, providerModels]) =>
        Object.entries(providerModels).flatMap(([model, preset]) =>
  ...
  /**
   * Retain the standing scope for a cold reader.
   *
   * Legacy file-backed preset generations deliberately live until the owning
   * process tree is disposed, so the lease disposer has no per-reader work.
   * @param id - the preset id, or `undefined` for {@link defaultId}.
