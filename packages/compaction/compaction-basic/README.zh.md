...
| `maxOutputContinuations` | `2` | 每条用户输入在输出截断后允许的后续轮次数；`0` 禁用输出恢复。需要 `auto: true`。 |
...
输出截断与上下文溢出不同。当最近的请求以 `max-tokens` 结束时，插件排队一个后续轮次，并显示 `Output limit reached; auto-continue N/LIMIT` 提示。插件从持久日志中统计最近一条用户消息之后的续接次数，包括已被压缩替换的历史。正常完成、取消、收件箱中已有输入或预算耗尽都会阻止自动续接。每个后续轮次记录自己的结束原因，因此恢复可以正常完成，而被截断的轮次仍保留记录。提示要求模型只继续未完成工作，不重复已完成工作，并完整重发仍然需要的截断工具调用。被截断的工具调用不会执行。达到上限只能证明输出被截断，不能证明任务尚未完成；模型可以直接给出简短最终答复。
