...
`delete(id)` 与写句柄争用同一个单写者 slot，因此当该 Session 仍有活动写入方时会拒绝。它只移除 `root` 下编码后 Session 自有的目录，绝不删除记录的 `cwd` 或项目文件；未知或已删除的 id 返回 `false`，成功删除的 id 可以重新创建。文件系统移除前会检查取消信号，无论成功或失败都会释放认领。
