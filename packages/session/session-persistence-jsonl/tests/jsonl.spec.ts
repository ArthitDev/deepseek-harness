...
describe('JsonlSessionPersistence: durability and crash semantics', () => {
  ...
  it('refuses deletion under another writer and removes all generations after release', async () => {
    const m = meta('delete-locked-generations', '/work')
    await writeLog(ctx.sessionPersistence, m, oneTurnLog())
    const owned = sessionDir(root, m.cwd, m.id)
    await writeFile(join(owned, 'session.v1.jsonl'), 'historical generation\n')
    const other = new Context()
    try {
      await other.plugin(JsonlSessionPersistence, { root, compression: 'none' })
      const writer = await other.sessionPersistence.open(m.id, 'write')
      try {
        await expect(ctx.sessionPersistence.delete(m.id)).rejects.toThrow(/owned/)
