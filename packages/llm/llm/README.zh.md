...
- **`GenerateOptions` 采样只包含 `temperature`／`maxTokens`／`stop`**——`toolChoice` 是独立的提供方无关工具调用策略；`top_p` 与 penalty 字段仍会等到有产生方落地后才加入（见[已删除惰性旋钮](../../../.agents/notes/archived/simplification/2026-07-04-drop-inert-request-knobs.md)）。
