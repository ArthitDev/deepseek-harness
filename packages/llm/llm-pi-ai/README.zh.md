...
此适配器通过 pi-ai 提供方、OpenAI 兼容网关与自托管端点路由 harness LLM 服务。每条已配置路由会在可用时继承 pi-ai 默认值；自定义路由只需配置。settings 与凭据按请求解析，因此更改无需重启即可生效。受支持的提供方登录使用授权服务，存储的凭据在跨进程锁下刷新。插件可以零路由启动，并在 settings 添加 profile 时激活路由。
