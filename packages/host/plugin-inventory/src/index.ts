...
import { findRow, globalEnablement, globalToggleAllowed, presetEnablement, readRows, revision } from './enablement.ts'
...
export class PluginInventoryGateway extends TypertRemoteService {
   ...
   * @param entryId - exact row id from the inventory.
   * @param moduleName - exact module specifier from the inventory.
   * @param preset - user preset id; omitted targets the current Host profile.
   * @returns current state, or a reason this row cannot be changed.
   */
  @Remote('edit')
  ...
  /**
   * Persist enablement with a revision check and an owner-only sibling backup.
   * Profile patches follow normal reload policy; preset changes apply to new sessions.
   * @param entryId - exact inventory row id.
   * @param moduleName - exact inventory module name.
   * @param enabled - requested literal state.
   * @param expectedRevision - revision read by the browser before confirmation.
   * @param preset - user preset id; omitted targets the current profile.
   * @returns the saved revision and state, not a runtime activation acknowledgement.
   */
  @Remote('setEnabled')
  async setEnabled(
