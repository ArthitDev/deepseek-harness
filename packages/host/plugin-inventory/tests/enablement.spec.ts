...
import { findRow, globalEnablement, globalToggleAllowed, presetEnablement, readRows, revision } from '../src/enablement.ts'
...
describe('plugin enablement documents', () => {
  it('preserves prompt strings and expressions when editing a nested plugin', () => {
    ...
    const result = readRows(presetEnablement(source, 'test', 'test-module', false))
    const row = findRow(result, 'test', 'test-module')
  ...
  it('rejects missing, duplicate, conditional and ancestor-disabled rows', () => {
    ...
    expect(() => readRows('[null]')).toThrow('array')
  })

  it.each(['[]\n', '[{id: other, disabled: true}]\n', '- id: other\n  disabled: true\n', '- id: other\n  disabled: true\n...\n'])(
    'stores a narrow override in supported YAML forms: %s', (source) => {
  ...
  it('retains comments in ordinary patch lists and rejects infrastructure toggles', () => {
    ...
    expect(globalToggleAllowed('approval', '@deepseek-ai/dsh-approval')).toBe(false)
